...
bxel_task::bxel_task(io::event_loop *loop, bxel_task_id id, int concurrent_num,
    ...
    : loop_(loop), client_(loop_), id_(id), url_(url), path_(path),
      concurrent_numbers_(concurrent_num), support_range_(false),
      state_(task_state::kNotStarted), recved_bytes_(0), total_bytes_(-1),
      file_(nullptr) {}

bxel_task::~bxel_task() { assert(state_ == task_state::kDone); }
...
bxel_task_id bxel_task::id() { return id_; }
...
void bxel::remove_task_in_loop(bxel_task_id id) {
  ...
  for (auto task = pending_task_list_.begin(); task != pending_task_list_.end();
       task++) {
    if ((*task)->id() == id) {
  ...
  for (auto task = working_task_list_.begin(); task != working_task_list_.end();
       task++) {
    if ((*task)->id() == id) {
...
static void report_progress(bxel_task_id id, size_t bytes_recved,
  ...
  printf("task:%d\tbytes recved: %zd\tbytes total:%zd\t%s\n", id, bytes_recved,
         bytes_total, error.message().c_str());
  loop.quit();
}
