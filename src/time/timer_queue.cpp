...
void timer_queue::push(const timer_ref &timer) { timer_queue_.push(timer); }

void timer_queue::pop() { timer_queue_.pop(); }

timer_ref timer_queue::top() { return timer_queue_.top(); }
...
int timer_queue::size() { return static_cast<int>(timer_queue_.size()); }
