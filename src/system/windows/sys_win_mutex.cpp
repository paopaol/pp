...
class MutexPrivate {
public:
  MutexPrivate() { InitializeCriticalSectionAndSpinCount(&cs_, 0); }
  ~MutexPrivate() { DeleteCriticalSection(&cs_); }
  void lock() { EnterCriticalSection(&cs_); }
  void unlock() { LeaveCriticalSection(&cs_); }

  CRITICAL_SECTION *getMutex() { return &cs_; }
...
Mutex::Mutex() : priv_(new MutexPrivate) {
...
void Mutex::lock() { priv_->lock(); }

void Mutex::unlock() { priv_->unlock(); }

void *Mutex::getMutex() { return (void *)priv_->getMutex(); }
...
public:
  once_flag_private() { InitOnceInitialize(&init_once_); }
  ~once_flag_private() {}
  PINIT_ONCE get_init_once() { return &init_once_; }
...
once_flag::once_flag() : priv_(new once_flag_private()) {}

once_flag::~once_flag() { delete priv_; }
...
void call_once(once_flag &flag, const call_once_routine &routine) {
  ...
  InitOnceExecuteOnce(flag.priv_->get_init_once(), InitHandleFunction,
                      (PVOID)&routine, &lpContext);
}
