...
namespace pp{
  ...
  namespace http{
    ...
    public:
      ...
      ResponseRef Get(const std::string &url, errors::Error &err){
      ...
      ResponseRef Post(const std::string &url, const std::string &cotent_type,
                       const std::string &body, errors::Error &err)
      {
        ...
        Defer	do_curl_reset((void *)NULL,
                            std::bind(curl_easy_reset, m_curl_handle));
        /* get it! */
        ret = curl_easy_perform(m_curl_handle);
        if (ret != CURLE_OK){
          err = errors::New(curl_easy_strerror(ret));
    ...
    private:
      ...
      int addHeaders(Headers &headers)
      {
        ...
        user_agent += "Mozilla/4.0 "
          "(compatible; MSIE 7.0; Windows NT 6.2; "
          "WOW64; Trident/7.0; .NET4.0E; .NET4.0C; "
          ".NET CLR 3.5.30729; .NET CLR 2.0.50727; "
          ".NET CLR 3.0.30729; HPDTDFJS)";
        ...
        for (auto h = headers.begin(); h != headers.end(); h++){
          ...
          m_curl_headers = curl_slist_append(m_curl_headers,
