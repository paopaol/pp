    ...
    namespace io {
            ...
            ,wakeup_ev_fd_(new iocp_event_fd(this, wakeup_pipe_.fd()))
        {
            ...
            event_poller_->Poll = [Poller_](int timeoutms,
                event_fd_list &gotEventFds, errors::error_code &error) {
                return  Poller_->poll(timeoutms, gotEventFds, error);
            };

            event_poller_->update_event_fd = [Poller_](event_fd *event, errors::error_code &error) {
                Poller_->update_event_fd(event, error);
            };
            event_poller_->remove_event_fd = [Poller_](event_fd *event, errors::error_code &error) {
        ...
        void event_loop::wakeup() 
        {
            ...
            HANDLE h = ::CreateFile(wakeup_pipe_.Name().c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL,
