...
void tcp_server::on_new_conn(int fd, const errors::error_code &error) {
  net::tcp_conn_ref conn = std::make_shared<net::tcp_conn>(loop_, fd, error);

  conn->connected(handle_new_conn_);
  conn->data_recved(handle_recved_data);
  conn->closed(
      [&](const net::tcp_conn_ref &conn, const errors::error_code &error) {
