...
tcp_conn::tcp_conn(io::event_loop *loop, int fd,
    ...
    : loop_(loop), socket_(AF_INET, SOCK_STREAM, fd),
      event_fd_(std::make_shared<io::iocp_event_fd>(loop_, fd)),
      state(Connecting), remote_("", -1), local_("", -1), bytes_written_(0),
      pending_read_(false) {
  event_fd_->data_recved(
      [&](errors::error_code &error) { read_done(errors::error_code()); });
  event_fd_->set_write_handler(
      [&](errors::error_code &error) { write_done(errors::error_code()); });

  event_fd_->closed(
      [&](errors::error_code &error) { close_done(errors::error_code()); });
...
void tcp_conn::close_done(const errors::error_code &error) {
  io::iocp_event_fd *evfd = static_cast<io::iocp_event_fd *>(event_fd_.get());
...
void tcp_conn::write_done(errors::error_code &error) {
  ...
  io::iocp_event_fd::io_request_ref active = evfd->remove_active_request();
  assert(active);
...
void tcp_conn::start_read(errors::error_code &error) {
  ...
  int ret = WSARecv(evfd->fd(), &request.get()->wasbuf, 1, &recvBytes, &flags,
...
void tcp_conn::connect_destroyed(const errors::error_code &error) {
  ...
  if (state == DisConnected) {
    if (connection_handler_) {
      connection_handler_(shared_from_this(), _time::now(), std::error_code());
    }
    errors::error_code err;
