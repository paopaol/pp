...
int tcp_connector::cancel_connect() {
  loop_->run_in_loop([&]() {
    ...
    DWORD bytes;
    int fd = evfd->fd();
    WSAIoctl(fd, SIO_BASE_HANDLE, NULL, 0, &fd, sizeof fd, &bytes, NULL, NULL);
    CancelIo((HANDLE)fd);
...
int tcp_connector::start_connect(_time::Duration timeout,
  ...
  evfd->enable_connect(std::bind(&tcp_connector::connect_done, this), error);

  auto request = evfd->create_io_request(io::iocp_event_fd::EV_CONNECT);
  int ret = connect_ex(evfd->fd(), (const sockaddr *)&addr, sizeof(addr), NULL,
  ...
  if (timeout > 0) {
    timer_ = _time::new_timer(_time::timer::timer_behavior::oneshot, timeout,
                              [&]() { cancel_connect(); });
  }
  return 0;
}
...
void tcp_connector::connect_done() {
  ...
  int ret = ::setsockopt(fd, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, nullptr, 0);
  if (ret < 0) {
