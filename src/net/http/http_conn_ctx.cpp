...
http_conn_ctx::http_conn_ctx(const http_request_ref &request,
    ...
    : request_(request), parser_(std::make_shared<http_parser>()), type_(type),
      parse_complete_(false), parse_header_complete_(false)
  ...
  case HTTP_RESPONSE: {
    if (ctx->resp_.body.is_nil()) {
      ...
      if (!ctx->some_body_.empty()) {
        ctx->resp_.body.write(ctx->some_body_.c_str(), ctx->some_body_.size(),
                              ctx->error_);
...
std::string http_conn_ctx::build_request_line() {
  ...
  std::string version = fmt::Sprintf("HTTP/%d.%d", request_->version.major,
...
int http_conn_ctx::parse_url() {
  ...
  int ret = http_parser_parse_url(request_->url.c_str(), request_->url.size(),
                                  false, &parse_url_);
  if (ret != 0) {
    return -1;
  }
